...
source "../colorscheme/tokyonight-moon.conf"
...
if [[ $CHANGED_COUNT -gt 0 ]]; then
  STATUS_CHANGED="${RESET}#[fg=#{@yellow},bg=#{@background},bold] ${CHANGED_COUNT} "
fi

if [[ $INSERTIONS_COUNT -gt 0 ]]; then
  STATUS_INSERTIONS="${RESET}#[fg=#{@green},bg=#{@background},bold] ${INSERTIONS_COUNT} "
fi

if [[ $DELETIONS_COUNT -gt 0 ]]; then
  STATUS_DELETIONS="${RESET}#[fg=#{@red},bg=#{@background},bold] ${DELETIONS_COUNT} "
fi

if [[ $UNTRACKED_COUNT -gt 0 ]]; then
  STATUS_UNTRACKED="${RESET}#[fg=#{@black},bg=#{@background},bold] ${UNTRACKED_COUNT} "
fi
...
case "$SYNC_MODE" in
1)
  REMOTE_STATUS="$RESET#[bg=#{@background},fg=#{@bred},bold] 󱓎" 
  ;;
2)
  REMOTE_STATUS="$RESET#[bg=#{@background},fg=#{@red},bold] 󰛃"
  ;;
3)
  REMOTE_STATUS="$RESET#[bg=#{@background},fg=#{@magenta},bold] 󰛀"
  ;;
*)
  REMOTE_STATUS="$RESET#[bg=#{@background},fg=#{@green},bold] "
  ;;
esac

if [[ -n $BRANCH ]]; then
  echo "$REMOTE_STATUS $RESET$BRANCH $STATUS_CHANGED$STATUS_INSERTIONS$STATUS_DELETIONS$STATUS_UNTRACKED"
  # echo "$REMOTE_STATUS $BRANCH $STATUS_CHANGED$STATUS_INSERTIONS$STATUS_DELETIONS$STATUS_UNTRACKED"
fi
